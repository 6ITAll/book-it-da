...
const schema = yup.object().shape({
  ...
  userId: yup.string().required('아이디를 입력해주세요').min(4, '아이디는 최소 4자 이상이어야 합니다'),
  phone: yup.string().required('전화번호를 입력해주세요').matches(/^[0-9]{10,11}$/, '올바른 전화번호 형식이 아닙니다'),
...
const Signup = (): JSX.Element => {
  ...
  const onSubmit = (data: any) => {
    ...
    const userInfo = { name: data.name, userId: data.userId, phone: data.phone, password: data.password };
    users.push(userInfo);
